...
{
	flags->crc32 = unaligned_read32le(in);
	flags->uncompressed_size = unaligned_read64le(&in[4]);
	flags->member_size = unaligned_read64le(&in[12]);
		...
		uint64_t memlimit, uint32_t flags)
{
	...
	if (flags & ~(LZMA_IGNORE_CHECK|LZMA_CONCATENATED))
